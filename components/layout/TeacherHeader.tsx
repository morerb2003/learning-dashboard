...
import { Plus, GraduationCap, Menu, ChevronDown, User, Settings } from "lucide-react";
...
export default function TeacherHeader({
  ...
  subtitle = "Manage courses, review assignments, and view student progress",
  user,
  onMenuToggle,
}: TeacherHeaderProps) {
  const [profileDropdownOpen, setProfileDropdownOpen] = useState(false);
  const displayName = user.full_name || user.email?.split("@")[0] || "Instructor";
    ...
    <header className="flex min-h-20 items-center justify-between border-b border-white/10 bg-zinc-950/60 px-4 py-4 backdrop-blur-xl md:px-8 shrink-0 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            <div className="h-8 w-8 rounded-full bg-gradient-to-tr from-emerald-600 to-teal-600 flex items-center justify-center font-bold text-white text-xs shadow-md shadow-emerald-500/20">
